...
router.post('/', async (req, res) => {
  const { address, token, amount, name } = req.body;
  ...
  if (!address || !token || !amount || !name) {
    return res
      ...
      .json({
        success: false,
        message: 'Missing information of this systemwallet',
      });
  }
...
router.put('/', async (req, res) => {
  const { name, address, token, amount } = req.body;
  // Simple validation
...
router.put('/update-balance', async (req, res) => {
  const { type, address, token, amount } = req.body;
  // Simple validation
  ...
  if (!originOne) {
    ...
    if (
      originOne.token !== token ||
      (type === '-' && amount > originOne.amount)
    )
      res.status(400).json({
        success: false,
        message:
          'Invalid token or token in system wallet is not enough to update!',
      });
...
router.delete('/:address', async (req, res) => {
  try {
    ...
    const DeletedTransaction = await SystemWallet.findOneAndDelete(
      DeleteCondition,
    );
