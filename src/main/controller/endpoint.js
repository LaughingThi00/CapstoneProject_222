...
router.post('/create-wallet', async (req, res) => {
  ...
  try {
    ...
    if (await User.findOne({ id })) throw new Error('User already exists');
...
router.post('/buy-crypto', async (req, res) => {
  ...
  try {
    ...
    if (!checkBillResult)
      ...
      const responseBill = await axios.post(`${Url.apiBackEndUrl}/bill`, {
        bill,
        partner: platform,
      });
...
router.put('/purchase', async (req, res) => {
  const { id, merchant, to_address, token, amount, network = 97 } = req.body;
  try {
    const result = await axios.post(
      `${Url.apiBlockChainUrl}/transaction/transfer`,
      {
        userId: id,
        merchant,
