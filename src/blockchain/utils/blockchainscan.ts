...
import { API_KEY_SCAN } from 'src/blockchain/constants/blockchain_scan';
import { BSC_API_KEY } from 'src/blockchain/constants/network';
...
export class BlockchainScan {
  ...
  static async fetTokenTransactions({ chainId, address, startBlock, toBlock }) {
    const url = API_KEY_SCAN[chainId].url;
    try {
      const response = await axios.get(url, {
        params: {
