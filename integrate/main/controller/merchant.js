...
router.post("/", async (req, res) => {
  ...
  if (!partner_code||!name) {
    return res
      .status(400)
      .json({ success: false, message: "Missing information of this merchant" });
  }
  ...
  try {
    ...
    if (merchant) {
      return res
        .status(400)
        .json({ success: false, message: "This merchant has existed already!" });
    }
    ...
    const newMerchant = new Merchant({
        partner_code,name,privateKey,publicKey
    });
...
router.put("/", async (req, res) => {
  ...
  try {
    UpdatedMerchant = await Merchant.findOneAndUpdate(UpdateCondition, originOne, {
      new: true,
    });
